...
use crate::dom::bindings::conversions::{jsstring_to_str, private_from_proto_check, DerivedFrom};
...
use crate::dom::bindings::inheritance::{Castable, TopTypeId};
...
use js::jsapi::{
    JS_ResolveStandardClass, JS_SetPrototype, JS_StringHasLatin1Chars, ObjectOpResult,
...
use js::rust::{
    Handle, HandleId, HandleObject, HandleValue, MutableHandleObject, MutableHandleValue,
};
use js::JS_CALLEE;
...
pub unsafe fn html_constructor_handling<T: DerivedFrom<Element> + DomObject>(
    cx: SafeJSContext,
    args: CallArgs,
    global: &Window,
    get_proto_object: fn(SafeJSContext, HandleObject, MutableHandleObject),
) -> bool {
    ...
    JS_SetPrototype(
        *cx,
        RawHandle::from(element.handle()),
        RawHandle::from(prototype.handle()),
    );
